...
int main(void) {
  ...
  while (y < 110) {
    ...
    while (lines[i]) {
      j = 0;
      while (lines[i][j]) {
        bzero(energy[i][j], sizeof(beam_dir));
        j++;
      }
      i++;
    }
    beam(lines, energy, y, 0, 0, 1);
    count = 0;
    i = 0;
    while (lines[i]) {
      j = 0;
      while (lines[i][j]) {
        count += energy[i][j]->down || energy[i][j]->right ||
                 energy[i][j]->up || energy[i][j]->left;
        j++;
      }
      i++;
    }
