...
int	max_repeat(char *s)
{
	...
	while (s[i] && s[i] != ' ')
	{
		card_repeat = 1;
		j = 1;
		while (s[i + j] != ' ' && is_in_pos(&s[i + j], s[i]) != -1)
		{
			card_repeat++;
...
int	cardcmp(char *h1, char *h2)
{
	...
	while (h1[i] == h2[i] && h1[i] != ' ')
		i++;
	return (is_in_pos("AKQJT98765432", h2[i]) - is_in_pos("AKQJT98765432", h1[i]));
...
int main(void) {
	...
	while (hands[i])
	{
		...
		printf("Hand : %s Worth : %d Count = %lld\n", hands[i], (i + 1) * ft_atoi(hands[i] + 5), count);
		i++;
	}
